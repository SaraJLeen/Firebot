...
export class LegacyEffectScriptExecutor {
    async canHandle(script: PluginBase | LegacyCustomScript): Promise<boolean> {
    ...
    async getScriptDetails(script: PluginBase | LegacyCustomScript): Promise<PluginDetails | null> {
        if (!this.isLegacy(script)) {
            return null;
        }
    ...
    async executeScript(
        ...
        const runRequest = buildRunRequest(null, manifest, parameters, context.trigger ?? {});
    ...
    private isLegacy(script: PluginBase | LegacyCustomScript): script is LegacyCustomScript {
        const hasNewManifest = (script as PluginBase).manifest != null;
