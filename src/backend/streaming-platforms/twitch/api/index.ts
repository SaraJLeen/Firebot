import { ApiClient, type HelixChannelUpdate, type HelixUser } from "@twurple/api";
...
class TwitchApi {
    ...
    constructor() {
        frontendCommunicator.onAsync("search-twitch-games", (query: string) => this.categories.searchCategories(query));
        ...
        frontendCommunicator.onAsync(
            "process-automod-message",
            async ({ messageId, allow }: { messageId: string, allow: boolean }) =>
                await this.moderation.processHeldAutoModMessage(messageId, allow)
        );

        frontendCommunicator.onAsync(
            "get-twitch-game",
            async (gameId: string) => await this.categories.getCategoryById(gameId)
        );
    ...
    private async refreshAccountData(accountType: "streamer" | "bot"): Promise<FirebotAccount> {
        const account =
            accountType === "streamer" ? AccountAccess.getAccounts().streamer : AccountAccess.getAccounts().bot;
